...
void HTN_B06_LedPB06(void * arg);
void HTN_B06_LedPB07(void * arg);
...
int main(void)
{
  ...
  Check_LCD_address(&hi2c1, 0x4E, 3, 100);
	...
	// Total HEAP mem = 3072
	xTaskCreate(HTN_B06_LedPB05,"Led PB05",64, NULL,
								tskIDLE_PRIORITY + 1, NULL);
	xTaskCreate(HTN_B06_LedPB06,"Led PB06",64, NULL,
								tskIDLE_PRIORITY + 1, NULL);
	xTaskCreate(HTN_B06_LedPB07,"Led PB07",64, NULL,
								tskIDLE_PRIORITY + 1, NULL);
	xTaskCreate(HTN_B06_LedPC13,"Led PC13",64, NULL,
								tskIDLE_PRIORITY + 1, NULL);
...
void Check_LCD_address(I2C_HandleTypeDef *hi2c_n, const uint8_t addr, int trails, int timeout)
{
	...
	{
			for (int i = 0; i<21; i ++) // check whether I2C address is correct?
			{
	...
	{
		for (int i = 0; i<5; i ++) // check whether I2C address is correct?
			{
...
void HTN_B06_I2C1LCD(void * arg){
	for(;;){
		...
		CLCD_I2C_WriteString(&LCD1, "HTN Day06:");
		CLCD_I2C_SetCursor(&LCD1, 0,1);
		CLCD_I2C_WriteString(&LCD1, "DUT");
		vTaskDelay(pdMS_TO_TICKS(MS_time));
		CLCD_I2C_Clear(&LCD1);
		CLCD_I2C_SetCursor(&LCD1, 0,0);
		CLCD_I2C_WriteString(&LCD1, "HTN Day06:");
		CLCD_I2C_SetCursor(&LCD1, 0,1);
		CLCD_I2C_WriteString(&LCD1, "ETE");
		vTaskDelay(pdMS_TO_TICKS(MS_time));
	}
}
